...
export default function MoroccanDestinations() {
  ...
  return (
    <section id="maroc" className="section-padding bg-gradient-to-b from-white via-[#fffaf2] to-[#edf8ff]">
      <div className="container mx-auto px-4">
        <div className="mx-auto mb-10 max-w-3xl text-center">
          <p className="mb-3 text-sm font-bold uppercase tracking-[0.24em] text-[#d86d2d]">Escapades au Maroc</p>
          <h2 className="font-['Playfair_Display'] text-4xl font-bold text-slate-900 md:text-5xl">Destinations Marocaines</h2>
          <p className="mt-4 text-base leading-7 text-slate-600 md:text-lg">Tanger, Casablanca, Rabat et Meknès : quatre ambiances, un patrimoine exceptionnel.</p>
        </div>

        <div className="mx-auto grid max-w-6xl gap-6 md:grid-cols-2 xl:grid-cols-4">
          {destinations.map((destination) => (
            <article key={destination.name} className="group overflow-hidden rounded-2xl bg-white shadow-lg shadow-slate-900/10 transition hover:-translate-y-1 hover:shadow-xl">
              ...
              <div className="p-5"><p className="text-sm leading-6 text-slate-600">{destination.description}</p><button type="button" onClick={() => setSelectedDestination(destination)} className="mt-4 text-sm font-bold text-[#d86d2d] underline-offset-4 hover:underline">Voir la destination</button></div>
            </article>
          ))}
        </div>
      </div>
      ...
      {selectedDestination && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 p-4" role="dialog" aria-modal="true" aria-label={`Destination ${selectedDestination.name}`} onClick={() => setSelectedDestination(null)}>
          <div className="relative w-full max-w-4xl overflow-hidden rounded-2xl bg-white p-3 shadow-2xl md:p-5" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="px-2 pb-1 pt-4 md:px-4"><p className="text-sm font-bold uppercase tracking-[0.18em] text-[#d86d2d]">{selectedDestination.landmark}</p><h3 className="mt-1 font-['Playfair_Display'] text-3xl font-bold text-slate-900">{selectedDestination.name}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{selectedDestination.description}</p></div>
          </div>
        </div>
      )}
    </section>
